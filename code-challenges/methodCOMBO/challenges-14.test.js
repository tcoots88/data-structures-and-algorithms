...
const toTitleCase = (arr) => {
  // Solution code here...
  return arr.map(index => index.charAt(0).toUpperCase() + index.slice(1, index.length));
};
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  let massive = arr.filter(character => Number(character.mass) > Number(arr[0].mass));
  return massive.map(str => str.name).join(' - ');
};
...
const sortBy = (property, arr) => {
  // Solution code here...
  return arr.sort((inputOne, inputTwo) => {
    if (property === 'price') {
      if (inputOne.price < inputTwo.price) return -1;
    } else if (property === 'name') {
      if (inputOne.name < inputTwo.name) return -1;
    }
  })
};
