...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  const total = input.map (element => {
    const removal = element.reduce ( (counter, value) => {
      if(value % 5 === 0 && typeof value !== 'string') {
        counter.push(Math.pow(2, value))
      }
      return counter;
    }, []);
    return removal;
...
let findMaleAndFemale = (data) => {
  ...
  data.forEach(elem => {
    if (elem.gender === 'female' || elem.gender === 'male') {
      counter++;
      if (counter > 1) {
        stringify = stringify + ' and ' + elem.name;
      } else {
        stringify = stringify + elem.name;
      }
    }
  })
  return stringify;
...
let findShortest = (data) => {
  // Solution code here...
  let shortestCharacter = data[0];
  for (let i = 1; i < data.length; i++) {
    if (parseInt(data[i].height) < parseInt(shortestCharacter.height)) {
      shortestCharacter = data[i];
    }
  }
